...
{
    return ((gmac_priv_t*)netdev_priv(dev))->rx_csum;
}

static int set_rx_csum(struct net_device* dev, u32 data)
{
    ((gmac_priv_t*)netdev_priv(dev))->rx_csum = data;
    return 0;
}
...
static void get_regs(struct net_device* dev, struct ethtool_regs* regs, void *p)
{
    ...
    spin_lock_irqsave(&priv->rx_spinlock_, irq_state);
...
#ifdef CONFIG_LEON_COPRO
    spin_unlock_irqrestore(&priv->rx_spinlock_, irq_state);
#else // CONFIG_LEON_COPRO
    ...
    printk("  available_for_read    %s\n", tx_available_for_read(&priv->tx_gmac_desc_list_info, 0) ? "yes" : "no");
