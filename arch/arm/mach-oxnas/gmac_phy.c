...
void set_phy_mode(struct net_device *dev)
{
    gmac_priv_t        *priv = (gmac_priv_t*)netdev_priv(dev);
    struct mii_if_info *mii = &priv->mii;
    struct ethtool_cmd *ecmd = &priv->ethtool_cmd;
    u32                 bmcr = mii->mdio_read(dev, mii->phy_id, MII_BMCR);

    if (priv->phy_type == PHY_TYPE_VITESSE_VSC8201XVZ) {
        // Allow s/w to override mode/duplex pin settings
        u32 acsr = mii->mdio_read(dev, mii->phy_id, VSC8201_MII_ACSR);
//printk("set_phy_mode() Allow s/w to override mode/duplex pin settings\n");
        acsr |= (1UL << VSC8201_MII_ACSR_MDPPS_BIT);
        mii->mdio_write(dev, mii->phy_id, VSC8201_MII_ACSR, acsr);
    }

    if (ecmd->autoneg == AUTONEG_ENABLE) {
        u32                 advert,  tmp;
        u32                 advert2 = 0, tmp2 = 0;
//printk("set_phy_mode() Auto negotiating link mode\n");
        ...
        if (advert != tmp) {
//printk("set_phy_mode() Setting MII_ADVERTISE to 0x%08x\n", tmp);
            mii->mdio_write(dev, mii->phy_id, MII_ADVERTISE, tmp);
            mii->advertising = tmp;
        }
        if ((mii->supports_gmii) && (advert2 != tmp2)) {
//printk("set_phy_mode() Setting MII_CTRL1000 to 0x%08x\n", tmp2);
        ...
        u32 tmp;
//printk("set_phy_mode() Unilaterally setting link mode\n");
