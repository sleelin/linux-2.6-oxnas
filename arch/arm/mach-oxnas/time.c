...
static irqreturn_t OXNAS_timer_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
#ifdef CONFIG_OXNAS_DDR_MON
...
void __init oxnas_init_time(void)
{
//printk("oxnas_init_time()\n");
...
{
    unsigned long ticks_since_last_intr = (unsigned long)TIMER_1_LOAD_VALUE -
                                          *((volatile unsigned long*)TIMER1_VALUE);

    // If there's a timer interrupt pending
    int timer_int_pending = *((volatile unsigned long*)RPS_IRQ_RAW_STATUS) & (1UL << TIMER_1_INTERRUPT);
