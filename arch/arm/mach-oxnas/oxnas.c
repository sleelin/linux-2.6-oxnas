...
static struct map_desc oxnas_io_desc[] __initdata = {
    ...
    { SDRAM_CTRL_BASE,      __phys_to_pfn(SDRAM_CTRL_BASE_PA),      SZ_4K,   MT_DEVICE }
#ifdef CONFIG_LEON_IMAGE_BASE
   ,{ LEON_IMAGE_BASE,      __phys_to_pfn(LEON_IMAGE_BASE_PA),      SZ_64K,  MT_DEVICE }
...
//printk("oxnas_mapio()\n");
    ...
    *(volatile u32*)DDR_AHB_REG = ((1UL << DDR_AHB_NO_RCACHE_ARMD_BIT)  |
    ...
    // Do NOT disable DDR HPROT for write coherency
    *(volatile u32*)DDR_AHB2_REG &= ~(((1UL << DDR_AHB2_IGNORE_HPROT_NUM_BITS) - 1) << DDR_AHB2_IGNORE_HPROT_BIT);
