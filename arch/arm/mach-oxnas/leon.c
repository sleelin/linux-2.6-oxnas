...
static u8* convert_adr_to_virt(u8* adr)
{
    ...
    // Force upper nibble to ARM memory map compatible value
    virt &= 0x0FFFFFFF;
    virt |= (SDRAM_PA & 0xF0000000);

    // Is address sane?
    if (virt < LEON_IMAGE_BASE_PA) {
        panic("CoPro SRAM load address 0x%08x below mapped region beginning at 0x%08x\n", (u32)adr, LEON_IMAGE_BASE_PA);
    } else {
...
static void leon_load_image(const s8 *srec)
{
    ...
    // Force the upper nibble to be 4 as this is the only valid value for the
    // ARM, but the LEON can use other values to affect endianness. Convert the
    // resulting physical address to a virtual address before attempting to get
    // the ARM to access it
...
void init_copro(const s8 *srec, unsigned long arg)
{
    ...
    *((u32*)(SRAM_BASE+SRAM_SIZE-sizeof(u32))) = arg;
