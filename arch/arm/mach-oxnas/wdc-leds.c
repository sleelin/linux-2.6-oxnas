 ...
 *
 * With a system clock of 25Mhz and the prescaler set to 16 we need
 * a load register value of 1627 to achieve 960Hz:
 *   25Mhz / 16 / 1627 = ~960
 */
#define FAST_TIMER_INT      (1627)  /* Timer2 count down      */
#define SYS_CLOCK       (25000000)  /* System clock frequency */
#define PRESCALE_VALUE        (16)  /* Value set in prescaler */
...
#define LED_MASK_ACT6  (1 << GPIO_13)  /* Activity 6 o'clock    */
#define LED_MASK_ACT9  (1 << GPIO_14)  /* Activity 9 o'clock    */
#define LED_MASK_FG12  (1 << GPIO_19)  /* Fuel Gauge 10 o'clock */
#define LED_MASK_FG2   (1 << GPIO_21)  /* Fuel Gauge 8 o'clock  */
...
/*
 * The following frame buffer holds one integer for each sub-cycle of a
 * full pulse width modulation cycle.  Each integer holds bits representing
 * which LEDs should be turned on/off during that sub-cycle.  The next_led
 * variable points to the current sub-cycle being processed.
 *
 * To achieve a desired duty cycle on a particular LED then you must set
 * the bit for that particular LED to one for the equivalent number of sub-
 * cycles.  For example, to get a 50% duty cycle on LED_MASK_ACT3 then you
 * must set the LED_MASK_ACT3 bit in half of the entries in frame_buffer to
 * one and the other half of the entries to zero.  Note that the function
 * set_led() performs this action.
 *
 * IMPORTANT!  There is an important design assumption about the frame
 * buffer.  The assumption is that all the LED bits for GPIO_A and GPIO_B
 * can be overlayed into one 32-bit integer without any overlapped bits.
 * The following #if tests that assumption at compile time.  Should you
 * ever get the associated error message during a compile then you must
 * review this design assumption and either make the frame buffer contain
 * two integers (one for GPIO_A and one for GPIO_B) or re-arrange your
 * GPIOs (i.e. get your hardware changed) so all the LEDs fit into one
 * integer.  There are other variables throughout this source code that
 * also rely on this assumption.  Perform a complete code review before
 * changing this assumption.
 */
#if ((LED_MASK_GPIO_A & LED_MASK_GPIO_B) != 0)
#  error LED frame buffer design assumption violated (see comments in source)
#endif
...
#define TRANS_STEP           (1)
#define SLEEP_HI             (0)
#define SLEEP_LO             (-LED100)
#define NUM_POWER_STEPS      (LED100)
 ...
 */
#define FAST_TPS       ((SYS_CLOCK / PRESCALE_VALUE) / FAST_TIMER_INT)
#define SLOW_COUNT     (30)
#define SLOW_TPS       (FAST_TPS / SLOW_COUNT)  /* want ~32Hz  */
   ...
   )
{
   ...
   if((next_led < frame_buffer) || (next_led >= frame_buffer + PWM_RESOLUTION))
   {
      next_led = frame_buffer;
   }
   ...
   /* Schedule the LED behavior state machine at a slower frequency */
   if(--slow_count < 0)
   {
...
static u32 get_inner_ring_bits(u16 value)
{
   ...
   if(value & (1 << 0))  pattern |= LED_MASK_FG2;
   if(value & (1 << 1))  pattern |= LED_MASK_FG4;
   if(value & (1 << 2))  pattern |= LED_MASK_FG6;
   if(value & (1 << 3))  pattern |= LED_MASK_FG8;
   if(value & (1 << 4))  pattern |= LED_MASK_FG10;
   if(value & (1 << 5))  pattern |= LED_MASK_FG12;
...
static void set_led(u32 led_bits, s8 value)
{
   ...
   for(i = 0; i < PWM_RESOLUTION; i++, fb++)
   {
      /* Zero out only the bits we are going set in a moment */
      *fb &= ~led_bits;

      /* Now set those bits to the desired value */
      if(negative_led_logic)
      {
         *fb |= (value > i ? led_bits : 0);
      }
      else
      {
         *fb |= (value > i ? 0 : led_bits);
      }
   }
}
...
void wdc_leds_behavior(unsigned long unused)
{
   ...
   switch(state)
   {
      ...
      case TRANS__4_UP:
         ramp2 -= TRANS_STEP;
         ramp1 += TRANS_STEP;
         if((ramp1 - TRANS_STEP) < SLEEP_HI) break;
         state++;
         break;
      ...
      case TRANS__3_DN:
         ramp2 += TRANS_STEP;
         ramp1 -= TRANS_STEP;
         if((ramp1 + TRANS_STEP) > -LED100) break;
...
{
   .name           = "wdc-leds:power",
   .brightness_set = wdc_leds_set_power,
};
...
{
   .name            = "wdc-leds:activity",
   .brightness_set  = wdc_leds_set_activity,
...
{
   .name           = "wdc-leds:ignore-act",
   .brightness_set = wdc_leds_set_ignore_activity,
};
...
{
   .name           = "wdc-leds:transition",
   .brightness_set = wdc_leds_set_transition,
};
...
{
   .name           = "wdc-leds:fuel-gauge",
   .brightness_set = wdc_leds_set_fuel_gauge,
};
...
{
   .name           = "wdc-leds:rebuilding",
   .brightness_set = wdc_leds_set_rebuilding,
};
...
{
   .name           = "wdc-leds:degraded",
   .brightness_set = wdc_leds_set_degraded,
};
...
{
   .name           = "wdc-leds:over-temp",
   .brightness_set = wdc_leds_set_over_temp,
};
...
/***************************************************************************/
static int wdc_leds_suspend(struct platform_device* pdev, pm_message_t state)
{
   int n = leds_created;
...
/***************************************************************************/
static int wdc_leds_resume(struct platform_device* pdev, pm_message_t state)
{
   int n = leds_created;
...
static int wdc_leds_probe(struct platform_device*  pdev)
{
   ...
   do
   {
      ...
      writel(LED_MASK_GPIO_A, GPIO_A_OUTPUT_ENABLE_SET);
      writel(LED_MASK_GPIO_B, GPIO_B_OUTPUT_ENABLE_SET);
      ...
      {
         rc = led_classdev_register(&pdev->dev, wdc_led_classes[leds_created]);
         if(rc < 0)
         {
            printk
            (
               KERN_ERR "failed to register led class \"%s\"\n",
...
{
   .probe    = wdc_leds_probe,
   .remove   = wdc_leds_remove,

#ifdef CONFIG_PM
   .suspend  = wdc_leds_suspend,
   .resume   = wdc_leds_resume,
#endif /* CONFIG_PM */
...
static int __init wdc_leds_init(void)
{
   ...
   printk
   (
      KERN_INFO "wdc-leds:  FAST_TPS=%d, SLOW_TPS=%d\n",
      FAST_TPS,
      SLOW_TPS
   );
   ...
   if(!ret)
   {
      wdc_leds = platform_device_register_simple("wdc-leds", -1, NULL, 0);
   }
