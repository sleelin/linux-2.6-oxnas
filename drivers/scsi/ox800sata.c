...
//#define SATA_TF_DUMP
//#define IGNORE_IDLE_ON_STOP_FAILURE
...
* PROTOTYPES
**************************************************************************/
static int  ox800sata_init_one(struct amba_device *, void *);
static int  ox800sata_remove_one(struct amba_device *);

static void ox800sata_port_disable(struct ata_port *);
static void ox800sata_dev_config(struct ata_port *, struct ata_device *);
...
static void ox800sata_post_set_mode(struct ata_port *ap);
...
static irqreturn_t ox800sata_irq_handler(int, void *, struct pt_regs *);
static void ox800sata_eng_timeout(struct ata_port *ap);
static void ox800sata_irq_clear(struct ata_port *);
static u32  ox800sata_scr_read(struct ata_port *ap, unsigned int sc_reg);
static void ox800sata_scr_write(struct ata_port *ap, unsigned int sc_reg,u32 val);
static int  ox800sata_port_start(struct ata_port *ap);
static void ox800sata_port_stop(struct ata_port *ap);
static void ox800sata_host_stop(struct ata_host_set *host_set);
...
typedef struct
{
    ...
    u32 int_status;
    u32 in_cleanup;    
#ifdef DEBUG_EOT_FAILURE
    oxnas_dma_channel_t* debug_dma_channel_;
    u32 debug_buffer_[512];
    u32 debug_phys_addr_;
    unsigned char* debug_src_address_;
    unsigned char* debug_dst_address_;
    unsigned long debug_length_;
#endif // DEBUG_EOT_FAILURE
} ox800sata_private_data;

/*
Uses a ficticious device id as the sata core doesn't have a real device id 
*/
static struct amba_id ox800sata_idtbl[] =
{/* { id , mask , data } */
    {  OX800SATA_DEVICEID , APB_BRIDGE_BASE_MASK , 0    },
    {} /* end of list */
};
...
static struct ata_port_operations ox800sata_port_ops =
{
    ...
    .ata_irq_on = ox800sata_irq_on,
    .ata_irq_ack = ox800sata_irq_ack,
    .pio_task = ox800sata_pio_task,
};
...
MODULE_DEVICE_TABLE(amba, ox800sata_idtbl);
...
{
    u8* link_base = (u8* )ap->host_set->mmio_base + 
 ...
 */
static int ox800sata_init_one(struct amba_device* pdev, void* port)
{
    
    struct ata_probe_ent* ent;
    unsigned char version;
#ifdef CONFIG_OX800SATA_DISK_LIGHT
    unsigned long reg;
#endif // CONFIG_OX800SATA_DISK_LIGHT
    
    DPRINTK("\n");
    
    /* check this is the right device */
    if (pdev->periphid != OX800SATA_DEVICEID)
    {
        printk(KERN_INFO"ox800sata_init_one: not right periphid\n");   
        return 0;
    }
    ...
    version = readl(((u32* )(pdev->res.start)) + OX800SATA_VERSION) & 0xff;
    switch (version)
    {
        case OX800SATA_VERS_921:
            printk(KERN_ERR"ox800sata: 921 based sata core not supported, recompile with CONFIG_SCSI_AV940SATA\n");
            return 0;
            break;
            
        case OX800SATA_VERS_924:
        case OX800SATA_VERS_NAS:
            printk(KERN_INFO"ox800sata: 924 based sata core.\n");   
            break;
            
        /* unknown version = bad */
        default:
            printk(KERN_ERR"ox800sata: unknown sata core (version register = 0x%02x)\n",version);     
            return 0;
            break;
    }
    ...
    /** @TODO scsi host template */
    ent->sht        = &ox800sata_sht;
    ...
    ent->mmio_base  = (void __iomem* )pdev->res.start;
    
    /** @todo the number of ports may be variable, 
              find out if this is a maxumum number */
              
    /* number of ports */
    ent->n_ports = 1;
    
    /* Define the interrupt line */
#if (CONFIG_ARCH_OXNAS_VERSION == 0)    
    ent->irq = ATA_INTERRUPT ;
#else
    ent->irq = pdev->irq[0] ;
#endif

    /* pointer to port access functions */
    ent->port_ops   = &ox800sata_port_ops;
 ...
 */
static int ox800sata_remove_one(struct amba_device* pdev)
{
    ...
    for (i = 0; i < host_set->n_ports; i++) 
    {
        ...
        /* free per port private data structure */
...
static int __init ox800sata_init( void )
{
    ...
    ret = amba_driver_register( &ox800sata_driver.bus );
...
{
    amba_driver_unregister( &ox800sata_driver.bus );
}
 ...
 */
static void ox800sata_dev_config(struct ata_port* ap, struct ata_device* pdev)
{
    u32 reg;
...
//        VPRINTK("(ignoring DRQ) ");    
    ...
    Reg = ox800sata_scr_read(ap, SCR_STATUS );
    if (!(Reg & 0x1)) { 
        result |= ATA_DF;
        result |= ATA_ERR;
    }
...
static void ox800sata_bmdma_setup(struct ata_queued_cmd *qc)
{
    ...
    /** @TODO Consider moving DMA channel request to an on port setup location */
    /* non-blocking request */    
    PrivateData->DmaChannel = oxnas_dma_request(0);
    
    /* check we got it, no option of returning an error code though :-( */
    if (PrivateData->DmaChannel == OXNAS_DMA_CHANNEL_NUL)
    {
        printk(KERN_ERR"Couldn't get a DMA channel for disk access.\n");
        return;
    }
    
    /* set callback */
    oxnas_dma_set_callback(PrivateData->DmaChannel,
                           OXNAS_DMA_CALLBACK_ARG_NUL,
                           (oxnas_callback_arg_t) qc->ap);
...
#ifdef DEBUG_EOT_FAILURE
    if (!oxnas_dma_is_active(PrivateData->debug_dma_channel_)) {
printk("Setting up\n");
        PrivateData->debug_length_ = sizeof(PrivateData->debug_buffer_) / 2;
        PrivateData->debug_src_address_ = (unsigned char*)(PrivateData->debug_phys_addr_);
        PrivateData->debug_dst_address_ = PrivateData->debug_src_address_ + PrivateData->debug_length_;
    } else {
printk("Still active\n");
        PrivateData->debug_length_ = 0;
    }
#endif // DEBUG_EOT_FAILURE
}
...
#ifdef DEBUG_EOT_FAILURE
        {
            ox800sata_private_data* PrivateData = (ox800sata_private_data*)qc->private_data;
            if (PrivateData->debug_length_) {
                printk("Starting...\n");
                // Setup and start the debug DMA immediately
                oxnas_dma_set(PrivateData->debug_dma_channel_,
                          PrivateData->debug_src_address_,
                          PrivateData->debug_length_,
                          PrivateData->debug_dst_address_,
                          OXNAS_DMA_MODE_INC,
                          OXNAS_DMA_MODE_INC,
                          0,
                          0);
...
static unsigned int  ox800sata_qc_issue(struct ata_queued_cmd *qc)
{
    ...
    private_data->port_disabled |=
        (! (ox800sata_scr_read(qc->ap, SCR_STATUS ) & 1 ) );
                                        ...
                                        struct pt_regs* regs)
{
...
#ifdef SATA_DEBUG
    orb2 = readl(ioaddr + OX800SATA_ORB2);
    printk("nsects = 0x%04x\n", orb2 & 0xffff);

    bbc = readl(ox800sata_get_bbp_base() + OX800SATA_BURST_CONTROL);
    int_status = readl(ioaddr + OX800SATA_INT_STATUS);
    dma_active = 0;
    is_write = 0;
    if (pd->DmaChannel) {
        dma_active = oxnas_dma_raw_isactive(pd->DmaChannel);
        is_write = (oxnas_dma_get_raw_direction(pd->DmaChannel) == 1);
    }

    printk("irq %d: pend=0x%08x, bbc=0x%08x -> %s\n", irq, int_status, bbc, is_write ? "write" : "read");
    if (pd->DmaChannel) {
        printk("dma %d %s idle\n", pd->DmaChannel->channel_number_, dma_active ? "is not" : "is");
    } else {
        printk("No DMA channel\n");
    }
    
    if (is_write && dma_active) {
    printk("WRITE PROBLEM\n");
    ...
    queue_work(ox800sata_driver.spot_the_end_q, &pd->spot_the_end_work);     
    return IRQ_HANDLED;
}
...
static void ox800sata_spot_the_end(void* anon)
{
    ...
    /* Poor quality SATA cables can cause errors to be reported even if there
    are no outstanding commands. Count these errors and report if we get too
    many. */
    if ((qc == NULL) ||
        !(PrivateData->int_status & OX800SATA_INT_END_OF_CMD) ) {
        DPRINTK(" qc=null\n");
    
/* This can be misleading if HW RAID is on as we occasionally get interrupts 
   from port 1 at a different time to port 0. There is no way of telling if a 
   RAID command has caused it as we can't see the command.
   
        ++PrivateData->ErrorsWithNoCommamnd;
        if (!((PrivateData->ErrorsWithNoCommamnd - 1) & (OX800SATA_NO_CMD_ERROR_RPT_COUNT - 1))) {
            printk(KERN_ERR"ox800sata: %d interrupts without a command, this is a symptom of faulty SATA cabling\n",
                PrivateData->ErrorsWithNoCommamnd);
        }
*/      
        return;
    ...
    if ( PrivateData->DmaChannel )
    {
        int busy = TRUE;
        DPRINTK(" wait for DMA to finish\n");

        /* wait for the DMA to finish */
        do {
            local_irq_save(flags);            
            if( !PrivateData->DmaChannel || 
                !oxnas_dma_is_active( PrivateData->DmaChannel ) )
            {
                busy = FALSE;
            }
        ...
        local_irq_save(flags);
        if (PrivateData->DmaChannel) {
            oxnas_dma_free( PrivateData->DmaChannel );
            PrivateData->DmaChannel = 0;
        }
        local_irq_restore(flags);
    }
...
static u32  ox800sata_scr_read(struct ata_port *ap, unsigned int sc_reg)
{
    ...
    return __ox800sata_scr_read(ioaddr, 0x20 + (sc_reg*4) );
}
...
static int  ox800sata_port_start(struct ata_port *ap)
{
    ...
    {
        ap->private_data = 
            kmalloc( sizeof(ox800sata_private_data), GFP_KERNEL );
    }
    ...
    pd = (ox800sata_private_data* )ap->private_data;
    INIT_WORK(&(pd->spot_the_end_work), &ox800sata_spot_the_end, ap);
...
#ifdef DEBUG_EOT_FAILURE
    pd->debug_dma_channel_ = oxnas_dma_request(1);
    pd->debug_phys_addr_ = dma_map_single(0, pd->debug_buffer_, pd->debug_length_, DMA_BIDIRECTIONAL);
    printk("Obtained debug DMA channel %d\n", pd->debug_dma_channel_->channel_number_);
    printk("Debug buffer virt = 0x%08x, phys = 0x%08x\n", (u32)pd->debug_buffer_, pd->debug_phys_addr_);
#endif // DEBUG_EOT_FAILURE
...
static void ox800sata_post_reset_init(struct ata_port* ap) 
{
    ...
    for (dev = 0; dev < ATA_MAX_DEVICES; ++dev) {
        if ( ap->device[dev].class == ATA_DEV_ATA )
            ox800sata_dev_config(ap, &(ap->device[dev]) );
    }
}
...
#ifdef DEBUG_EOT_FAILURE
{
    ox800sata_private_data* pd ;
    pd = (ox800sata_private_data* )ap->private_data;
    dma_unmap_single(0, pd->debug_phys_addr_, pd->debug_length_, DMA_BIDIRECTIONAL);
	oxnas_dma_free(pd->debug_dma_channel_);
    pd->debug_dma_channel_ = 0;
}
#endif // DEBUG_EOT_FAILURE
...
static void ox800sata_pio_task(void* _data)
{
    ...
    direction = (qc->dma_dir == DMA_FROM_DEVICE) ?
    ...
    if ( !PrivateData->DmaChannel ) {
        ...
        /* setup a scatter gather dma */
        oxnas_dma_device_set_sg(PrivateData->DmaChannel,
                                direction,
                                qc->__sg,
                                qc->n_elem,
                                &oxnas_sata_dma_settings,
    ...
    if( oxnas_dma_is_active( PrivateData->DmaChannel ) ) {
        ...
        /* try again in 1 ms */
		ata_port_queue_task(ap, ox800sata_pio_task, ap, (HZ / 1000) );
        return;
    }
...
static void ox800sata_bmdma_stop(struct ata_queued_cmd *qc)
{
    ...
    if (private_data->DmaChannel) {
        ...
        /* Free the dma channel */
        oxnas_dma_free(private_data->DmaChannel);
        private_data->DmaChannel = 0;
    }
    local_irq_restore(flags);
}
 ...
 */ 
#if (CONFIG_ARCH_OXNAS_VERSION == 0)    
struct amba_device ox800sata_dev = 
{
    .periphid = OX800SATA_DEVICEID,
    .res =
    {
        .start = ATA_BASE,
        .end = (ATA_BASE + 64), /* space for 64 registers = 256 bytes */
    },
    .dev =
    {
        .bus_id = "ox800sata_sata_device",
        .coherent_dma_mask = 0xffffffff,
    },
    .irq = { ATA_INTERRUPT, NO_IRQ },
}; 
#else
struct amba_device ox800sata_dev1 = 
{
    .periphid = OX800SATA_DEVICEID,
    .res =
    /* 
     * claim the bus space between SATA_REGS_BASE and SATA_LINK_REGS_BASE + 64
     * as our own (within the confines of the AMBA bus driver) 
     */
    {
    ...
    .dev =
    {
        .bus_id = "ox800sata_port0",
        .coherent_dma_mask = 0xffffffff,
    },
    .irq = { SATA_1_INTERRUPT, NO_IRQ },
}; 
struct amba_device ox800sata_dev2 = 
{
    .periphid = OX800SATA_DEVICEID,
    .res =
    /* 
    ...
    .dev =
    {
        .bus_id = "ox800sata_port1",
        .coherent_dma_mask = 0xffffffff,
    },
    .irq = { SATA_2_INTERRUPT, NO_IRQ },
}; 
#endif
...
static int __init ox800sata_device_init( void )
{
    ...
    {
        // check if the memory mapped ata device is not already in use    
        if (check_mem_region(ox800sata_dev1.res.start, (ox800sata_dev1.res.end - ox800sata_dev1.res.start)))
        {
            DPRINTK(" port 0 device memory already in use\n");
            return -EBUSY;
        }
        
        // register the ata device for the driver to find
        ret = amba_device_register( &ox800sata_dev1, &iomem_resource );
        DPRINTK(" %i\n", ret);
    }
    
#ifndef CONFIG_OX800SATA_SINGLE_SATA
    {
        // check if the memory mapped ata device is not already in use    
        if (check_mem_region(ox800sata_dev2.res.start, (ox800sata_dev2.res.end - ox800sata_dev2.res.start)))
        {
            DPRINTK(" port 1 device memory already in use\n");
            return -EBUSY;
        }
        
        // register the ata device for the driver to find
        ret = amba_device_register( &ox800sata_dev2, &iomem_resource );
        DPRINTK(" %i\n", ret);
    }
#endif /* CONFIG_OX800_SINGLE_SATA */
...
{
    amba_device_unregister( &ox800sata_dev1 );
    amba_device_unregister( &ox800sata_dev2 );
}
 ...
 */
int  ox800sata_RAID_faults( void ) {
 ...
 */
int ox800sata_get_port_no(request_queue_t* q)
{
 ...
 * same LBA size.
 */
int ox800sata_LBA_schemes_compatible( void )
{
...
EXPORT_SYMBOL( ox800sata_RAID_faults );
EXPORT_SYMBOL( ox800sata_get_port_no );
EXPORT_SYMBOL( ox800sata_LBA_schemes_compatible );
