...
typedef struct gmac_dma_desc
{
    ...
    u32 buffer2;
    /** Storage for pointer to the socket buffer encapsulating the data buffers */
    u32 skb;
    /** Storage for pointer to SRAM buffer info if used instead of skb */
    u32 sram_info;
...
#define GMAC_DESC_ALLOC_SIZE        ((NUM_GMAC_DMA_DESCRIPTORS) * (6*4))
