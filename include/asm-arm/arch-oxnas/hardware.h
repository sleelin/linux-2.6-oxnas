...
#ifdef CONFIG_LEON_IMAGE_BASE
#define LEON_IMAGE_SIZE         (CONFIG_LEON_PAGES * PAGE_SIZE)
#define LEON_IMAGE_BASE_PA      (CONFIG_LEON_IMAGE_BASE)
#else // CONFIG_LEON_IMAGE_BASE
#define LEON_IMAGE_SIZE     	0
#define LEON_IMAGE_BASE_PA	0
#endif // CONFIG_LEON_IMAGE_BASE
...
#if defined(CONFIG_DESCRIPTORS_IN_SRAM) && defined(LEON_IMAGE_IN_SRAM)
...
#define SRAM_NET_BUFFER_AVAIL_MEM (SRAM_SIZE - LEON_IMAGE_SIZE - DESCRIPTORS_SIZE)
#else
#define SRAM_NET_BUFFER_AVAIL_MEM (SRAM_SIZE - DESCRIPTORS_SIZE)
#endif
#define SRAM_NET_BUFFER_BASE_PA   (DESCRIPTORS_BASE_PA + DESCRIPTORS_SIZE)
#define SRAM_NET_BUFFER_BASE_VA   (DESCRIPTORS_BASE    + DESCRIPTORS_SIZE)
...
#define SDRAM_CTRL_BASE_PA     0x4A000000
...
/* PCI divider should divide NOMINAL_PLL400_FREQ which occasionally isn't 400MHz
the divider divides by 2(n + 1) to get 33MHz*/
#define PCI_CLOCK_DIVIDER (((NOMINAL_PLL400_FREQ) / (2 * NOMINAL_PCICLK)) - 1)

/* RPS timer setup */
#define TIMER_INPUT_CLOCK      NOMINAL_RPSCLK_FREQ
...
#define TIMER_1_LOAD_VALUE     (TIMER_1_PRESCALED_CLK / CLOCK_TICK_RATE)
#define TICKS_TO_US_SCALING    1024
#define TICKS_TO_US_FACTOR     (((2 * TICKS_TO_US_SCALING * 1000000) + TIMER_1_PRESCALED_CLK) / (2 * TIMER_1_PRESCALED_CLK))
